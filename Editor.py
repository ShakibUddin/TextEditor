...
def countVowels():
    input=textBox.get("1.0","end")#1.0 means first character , end means till end
...
def countSpecialCharacters():
    ...
    for letter in input:
        if letter.isalpha()==False and letter.isdigit()==False and letter.isspace()==False:
            specialCharacters+=1
    tk.messagebox.showinfo(title="Special Characters", message=f"There are {specialCharacters} special characters in this text")
...
def countSpecificCharacter():
    ...
    def getChar():
        ...
        for letter in input:
            if (letter == charcter):
                specificCharacter += 1
        tk.messagebox.showinfo(title="Specific Character",message=f"\'{charcter}\' appeared {specificCharacter} times in this text")
...
def replaceWord():
    ...
    def getWord():
        ...
        if specificWord>0:
            tk.messagebox.showinfo(title="Word Replacement",message=f"\"{word1}\" replaced with {replaceword} in {specificWord} places")
        else:
